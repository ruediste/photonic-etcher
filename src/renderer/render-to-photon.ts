...
export default async function renderToPhoton(layers: (StackupLayer & { displayOrder: number, inverted: boolean })[], options: ExportOptions,
    ...
    for (const layer of layers) {
        ...
        if (options.exposureCalibration === undefined) {
            outputFileLayers.push({ exposureTime: options.exposureTimes[layer.id], data: ctx.getImageData(0, 0, outputResolution[0], outputResolution[1]).data });
        }
        else {
            ...
            for (let row = 0; row < options.exposureCalibration.rows; row++) {
                for (let column = 0; column < options.exposureCalibration.columns; column++) {
                    ...
                    let outputLayerExposureTime = step === 0 ? times[0] : times[step] - times[step - 1];
                    outputFileLayers.push({ exposureTime: outputLayerExposureTime, data: ctx.getImageData(0, 0, outputResolution[0], outputResolution[1]).data });
